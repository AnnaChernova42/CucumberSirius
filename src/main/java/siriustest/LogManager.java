...
public class LogManager {
    ...
    private static String getSessionToken(FileInputStream fis) throws IOException {
        ...
        // TODO: 21.04.2017 Костыль, берем не весь токен, а его часть (последние 20 символов). Когда СБТ исправит маскирование токена, убрать.
        token = token.substring(25);
        return token;
    }

    private static String getSessionTokenPN(FileInputStream fis) throws IOException {
            BufferedReader reader = new BufferedReader(new InputStreamReader(fis, "UTF-8") );
            String token = "CMD_START";
    ...
    public static boolean parseServerLogPN( File serverLog, String stringToFind) throws IOException {
        String WASSessionId = "___NO_WAS_SESSION_ID___";
        try (FileInputStream serverFis = new FileInputStream(serverLog)) {
            // Находим сразу WASSessionId сессии в клиентском логе, только для платежей наличными, т к токена нет
            WASSessionId = getSessionTokenPN(serverFis);
        }
