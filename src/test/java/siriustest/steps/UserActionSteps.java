...
public class UserActionSteps {
    ...
    @Тогда("^в логе ПН сервера присутствует строка \"([^\"]*)\"$")
    public void serverLogShouldContainStringPN(String searchString) throws Throwable {
        boolean found = LogManager.parseServerLogPN(
                new File( PROPERTIES.getProperty("log.server") ),
                searchString);
